...
const test = async () => {
  ...
  const tokenByIndex5 = await cep47.getTokenByIndex(
    KEYS.publicKey,
    indexByToken5
  );
  ...
  const transferOneRecipient = CLPublicKey.fromHex(
    "016e5ee177b4008a538d5c9df7f8beb392a890a06418e5b9729231b077df9d7215"
  );
  ...
  console.log(
    `...... Transfer from ${KEYS.publicKey.toAccountHashStr()} to ${transferOneRecipient.toAccountHashStr()}`
  );
  ...
  const transferFromRecipient = CLPublicKey.fromHex(
    "019548b4f31b06d1ce81ab4fd90c9a88e4a5aee9d71cac97044280905707248da4"
  );

  console.log(
    `...... Transfer from ${KEYS.publicKey.toAccountHashStr()} to ${transferFromRecipient.toAccountHashStr()}`
  );
